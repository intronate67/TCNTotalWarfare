...
public class GameManager {
	...
	public void setup(TotalWarfare plugin) {
		p = plugin;
		LoadGames();
		LoadKits();
		for (Game g: getGames()) {
	...
	public void LoadGames() {
		...
		games().clear();
		int no = c.getInt("tw-system.arenano", 0);
		int loaded = 0;
		int a = 1;
		while (loaded < no) {
			if (c.isSet("tw-system.arenas." + a + ".x1")) {
				//c.set("tw-system.arenas."+a+".enabled",c.getBoolean("tw-system.arena."+a+".enabled", true));
				if (c.getBoolean("tw-system.arenas." + a + ".enabled")) {
					//TotalWarfare.$(c.getString("tw-system.arenas."+a+".enabled"));
					//c.set("tw-system.arenas."+a+".vip",c.getBoolean("tw-system.arenas."+a+".vip", false));
					TotalWarfare.$("Loading Arena: " + a);
					loaded++;
	...
	public void createArenaFromSelection(Player pl) {
		...
		int no = c.getInt("tw-system.arenano") + 1;
		c.set("tw-system.arenano", no);
		if (games().size() == 0) {
			no = 1;
		} else no = games().get(games().size() - 1).getID() + 1;
		SettingsManager.getInstance().getSpawns().set(("spawns." + no), null);
		c.set("tw-system.arenas." + no + ".world", max.getWorld().getName());
		c.set("tw-system.arenas." + no + ".x1", max.getBlockX());
		c.set("tw-system.arenas." + no + ".y1", max.getBlockY());
		c.set("tw-system.arenas." + no + ".z1", max.getBlockZ());
		c.set("tw-system.arenas." + no + ".x2", min.getBlockX());
		c.set("tw-system.arenas." + no + ".y2", min.getBlockY());
		c.set("tw-system.arenas." + no + ".z2", min.getBlockZ());
		c.set("tw-system.arenas." + no + ".enabled", true);
	...
	public void hotAddArena(int no) {
