package me.intronate67.TotalWarfare.logging;
...
public class QueueManager {
	...
	public void setup(){
		baseDir = new File(TotalWarfare.getPluginDataFolder()+"/ArenaData/");
		try{
			if(!baseDir.exists()){
				baseDir.mkdirs();
			}
		...
		public void run(){
			...
			if(data != null){
				...
				if(a != -1){
					...
					TotalWarfare.$ ("Arena "+id+" reset. Rolled back "+(totalRollback+rb)+" blocks in "+iteration+" iterations ("+pt+" blocks per iteration Total time spent rolling back was "+time+"ms)");
					game.resetCallback();
				}
			}else{TotalWarfare.$ ("Arena "+id+" reset. Rolled back "+totalRollback+" blocks in "+iteration+" iterations. Total time spent rolling back was "+time+"ms");
				game.resetCallback();
			}
		}
