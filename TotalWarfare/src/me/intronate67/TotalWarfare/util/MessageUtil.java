	...
	public static String replaceVars(String msg, HashMap<String, String>vars){
		...
		for(String s:vars.keySet()){
			try{
				...
				TotalWarfare.$(Level.WARNING, "Failed to replace string vars. Error on "+s);
				error = true;
			}
		}
		if(error){
			TotalWarfare.$(Level.SEVERE, "Error replacing vars in message: "+msg);
			TotalWarfare.$(Level.SEVERE, "Vars: "+vars.toString());
			TotalWarfare.$(Level.SEVERE, "Vars Cache: "+varcache.toString());
		}
		return msg;
	}
	...
	public static String replaceVars(String msg, String[] vars){
		...
		for(String str: varcache.keySet()){
			try{
				...
				TotalWarfare.$(Level.WARNING,"Failed to replace string vars. Error on "+str);
				error = true;
			}
		}
		if(error){
			TotalWarfare.$(Level.SEVERE, "Error replacing vars in message: "+msg);
			TotalWarfare.$(Level.SEVERE, "Vars: "+Arrays.toString(vars));
			TotalWarfare.$(Level.SEVERE, "Vars Cache: "+varcache.toString());
		}
